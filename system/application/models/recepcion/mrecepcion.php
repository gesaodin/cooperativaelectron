...
class MRecepcion extends Model {
    ...
    public function listar($arr){
        ...
        $oCabezera[5] = array("titulo" => "Cedula", "atributos" => "width:100px",'buscar'=>0);
        $oCabezera[6] = array("titulo" => "Cliente", "atributos" => "width:100px", "buscar" => 0);
        $oCabezera[7] = array("titulo" => "Fecha Rec.");
        $oCabezera[8] = array("titulo" => "Hora", "atributos" => "width:100px", "buscar" => 0);
        $oCabezera[9] = array("titulo" => "Observacion", "atributos" => "width:100px");
        $oCabezera[10] = array("titulo" => "Posesion", "atributos" => "width:100px");
        if($arr['estatus'] == 0 && ($usu == 0 || $usu == 27 || $usu == 28)){
            $oCabezera[10]['tipo'] = "combo";
            $oCabezera[11] = array("titulo" => "AC", "tipo" => "bimagen", "funcion" => 'aceptarEstatusDocu', "parametro" => "1,2,10", "ruta" => __IMG__ . "botones/aceptar1.png", "atributos" => "width:10px");
        }
        ...
        if ($iCantidad > 0) {
            ...
            foreach ($Conexion as $row) {
                ++$i;
                $oFil[$i] = array("1" => $row -> oidrec,"2"=>$arr['estatus'] ,"3" => $row -> envia, "4" => $row -> recibe, "5" => $row -> cedula, "6" => $row -> nombre,
                    "7" => $row -> fec, "8" => $row -> hora, "9" => $row -> observacion , "10" => $row -> descripcion);
                if($arr['estatus'] == 0 && ($usu == 0 || $usu == 27 || $usu == 28)){
                    $oFil[$i]["11"] = "";
                }
            }
