...
function Calcular_Compuesto(item) {
	...
	var compu1 = $("#cmp1").val();
	if(valor < compu1){
		alert("Numero de cuotas no debe ser mayor a las cuotas del presupuesto..");
		return false;
	}
	if(compu1 == 0) $("#cmp1 > option[value=12]").attr("selected","selected");
	var compu1 = $("#cmp1").val();
	$("#cmp2").val(valor-compu1);
	ct = Math.round(monto_final / valor);
	if($("#txtCmp1").val()=='') $("#txtCmp1").val(ct);
	//alert(monto_final);
	resta = monto_final - Math.round(ct*valor);
	if (resta > 0) {
		agrega_cuota = parseInt(resta) + parseInt(tiene);
		$("#txtMT1").val(agrega_cuota);
		monto_final += resta;
	...
	//alert(monto_final);
	monto_n1 = $("#txtCmp1").val() * $("#cmp1").val();
	ct2 = Math.round((monto_final - monto_n1)/$("#cmp2").val());
	$("#txtCmp2").val(ct2)
	monto_n2 = $("#txtCmp2").val() * $("#cmp2").val();
	$("#txtMTCmp1").val(monto_n1);
	$("#txtMTCmp2").val(monto_n2);
