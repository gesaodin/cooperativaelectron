...
window.webcam = {
	...
	get_movie: function() {
		...
		if (!this.loaded) return alert("ERROR: No Se pudo cargar la camara");
		var movie = document.getElementById('webcam_movie');
		if (!movie) alert("ERROR: No Se Encontro Camara");
		return movie;
	},
		...
		switch (type) {
			...
			case 'error':
				...
				if (!this.fire_hook('onError', msg)) {
					alert("No se puede detectar la Camara, por favor verifique que este conectada.");
				}
				break;
