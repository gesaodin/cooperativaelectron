...
var TGrid = function(oEsq, div, strTitulo) {
	...
	this.Generar_Json = function() {
		...
		/*if(this.EstiloC ==  ''){
			...
			div_medio.style.cssText = this.EstiloC;
		}*/
		...
		this.div_inferior.id = "div_inferior" + this.GetName;
		div_principal.appendChild(this.div_inferior);
		
		
		//var la_tabla = document.createElement("table");
		this.la_tabla.id = "tabla_tgrid" + this.GetName;
		if(this.Estilo != ''){
		...
		$.each(oEsquema.Cabezera, function(sId_Columnas, sArreglo) {
			...
			if(sArreglo.buscar != null) { //VERIFICA SI EN LA COLUMNA LLEVA FILTRO DE BUSQUEDA
				...
				/*Crear_Evento(filtro, "keyup", function() {
					self.filtra(this.value,self.la_tabla, pos);
				});*/
				filtro.onblur = function() {
					Oculta_Filtro(filtro);
				}
				/*Crear_Evento(filtro, "blur", function() {
					Oculta_Filtro(filtro);
				});*/
					...
					if( typeof arreglo_elementos[sCol] != "string") {
						...
						switch(nuevo.type) {
							case 'button':
								...
								break;
							case 'image':
								if(this.Detalle == 1 && oEsquema.Cabezera[sCol].tipo == 'detallePre'){
									columna_detalle.innerHTML = sValor;
									nuevo.onclick = function() {
										muestra_detalle(fila2.id, self.la_tabla.id,nuevo.id);
									}
								}
								if(this.Detalle2 == 1 && oEsquema.Cabezera[sCol].tipo == 'detallePost'){
									nuevo.onclick = function() {
										muestra_detalle(fila2.id, self.la_tabla.id,nuevo.id);
										muestra_detalle_post(oEsquema.Cabezera[sCol].funcion, fila.id, oEsquema.Cabezera[sCol].parametro, self.la_tabla.id, tipo_detalle);
									}
								}
		...
		if(_exel == true){
			this.div_inferior.appendChild(xls);	
		}
		
		if(oEsquema.Editable != '' && oEsquema.Editable != null){
			this.div_inferior.appendChild(editar);
		}
